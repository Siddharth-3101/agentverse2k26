...
const AllEvents = () => {
  // Navigation active tab: 'explore' | 'participated'
  ...
  const filteredEvents = DUMMY_EVENTS.filter((evt) => {
    const matchesSearch =
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            <div className="flex items-center space-x-3">
              <div className="bg-gradient-to-tr from-[#1c4980] to-[#2563eb] text-white p-2 rounded-xl shadow-md font-bold text-xl flex items-center justify-center w-10 h-10">
                A
              </div>
              <div>
                <span className="font-extrabold text-xl text-[#1c4980] tracking-tight">AgentVerse</span>
                <span className="text-xs text-slate-500 font-medium ml-2 hidden sm:inline-block">Events Portal</span>
              </div>
            </div>

            {/* View Switcher: Explore Events vs Participated Events */}
            <div className="flex items-center bg-slate-100 p-1 rounded-xl border border-slate-200">
              <button
                ...
                className={`px-4 py-2 rounded-lg text-sm font-semibold transition-all duration-200 flex items-center space-x-2 ${
                  activeTab === 'explore'
                    ? 'bg-white text-[#1c4980] shadow-sm'
              ...
              >
                ...
                <span className="bg-emerald-100 text-emerald-800 text-xs px-2 py-0.5 rounded-full font-bold">
                  {DUMMY_PARTICIPATED_EVENTS.length}
                </span>
              </button>
            </div>
        ...
        {activeTab === 'explore' && (
          <div>
            ...
            <div className="mb-6">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                  <h1 className="text-3xl font-extrabold text-slate-900 tracking-tight flex items-center gap-2">
                    {filteredEvents.length}+ Events & Opportunities
                    <span className="inline-block w-2.5 h-2.5 bg-emerald-500 rounded-full animate-pulse"></span>
                  </h1>
                  <p className="text-slate-600 mt-1 font-medium">
                    Build, code, innovate and showcase your skills in campus and online events.
                  </p>
                </div>
              ...
              {/* Category Pills Navigation (Unstop inspired bar) */}
            ...
            {/* Layout Grid: Left Cards List + Right Featured Sidebar */}
            ...
            <div className="mb-6">
              <h1 className="text-3xl font-extrabold text-slate-900 tracking-tight flex items-center gap-2">
                ...
                <span className="text-xs font-bold bg-emerald-100 text-emerald-800 px-2.5 py-1 rounded-full">
                  {filteredParticipatedEvents.length} Active Registrations
                </span>
              </h1>
              <p className="text-slate-600 mt-1 font-medium">
                Track all hackathons, quizzes, and workshops you have registered for or completed.
              </p>

              {/* Search Bar for Participated Events */}
              <div className="mt-4 max-w-md">
                <input
                  ...
                  value={searchQuery}
                  onChange={(e) => setSearchQuery(e.target.value)}
                  className="w-full px-4 py-2.5 bg-white border border-slate-300 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[#1c4980]"
                />
              </div>
            </div>
